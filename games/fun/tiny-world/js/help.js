...
TW.Help = (function () {
  ...
  /* ---------- 갇힘 확인 ----------
     예전 판에서 건물에 둘러싸여 저장된 경우를 위한 안전장치.
     지금은 그렇게 지을 수 없지만, 이미 갇힌 저장은 스스로 빠져나올 수 있어야 한다. */
  var trapCheck = 0, trapAsked = false;

  function checkTrapped(dt) {
    trapCheck += dt;
    if (trapCheck < 2) return;
    trapCheck = 0;
    if (!TW.Building.trapped()) { trapAsked = false; return; }
    if (trapAsked || TW.UI.currentPanel()) return;
    trapAsked = true;
    TW.Audio.play('event');
    TW.UI.cheer('<div class="c-ic">🚪</div><h2>어? 길이 막혔네!</h2>' +
      '<p>둘러싸여서 밖으로 나갈 수 없어요.<br>바로 옆 건물을 부수고 재료를 돌려줄게요.</p>' +
      '<button class="go" style="width:100%;margin-top:10px" data-act="rescue">밖으로 나가기</button>');
  }
  ...
  var TOPICS = [
    ...
    { id: 'build', icon: '🏠', title: '건물 짓기',
      lines: ['아래 🏠 **건설**을 누르고, 짓고 싶은 건물의 "짓기"를 눌러.',
              ...
              '**길이 막히는 자리는 반짝이지 않아.** 건물로 나를 가둘 수 없게 되어 있어서 안심해도 돼.',
              '잘못 지었으면 건설 맨 아래 **내가 지은 건물 정리**에서 부수면 재료를 전부 돌려받아.',
    ...
    { id: 'stuck', icon: '🤔', title: '이럴 때는 어떻게 해요?',
      lines: ['**"활동력이 없어요"** → 집에서 쉬거나, 가방에서 🍓 열매·🍄 버섯을 먹어. 가만히 있어도 저절로 차.',
              ...
              '**"건물을 잘못 지었어요"** → 🏠 건설 맨 아래 **내가 지은 건물 정리**에서 부수면 돼. 재료는 전부 돌려받아!',
              '**"건물에 막혀서 못 지나가요"** → 길이 막히는 자리에는 아예 지어지지 않아. 그래도 답답하면 건물을 부수면 돼.',
